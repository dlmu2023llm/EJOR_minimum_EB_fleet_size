...
def column_generation(para, unfinished_task, charging_count):
    ...
    charging_arc = [(i, j) for i, j in para['charging_arc']]
    ...
    while True:
        ...
        relax_model.setObjective(obj, GRB.MINIMIZE)
        ...
        if sub_model.status == GRB.OPTIMAL:
            ...
            for i, j in vehicle_arc:
                ...
                trip_num = sum(1 for i, j in operation_arc if X[i, j].x >= 0.5)
            B += 1
